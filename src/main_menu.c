...
// Pokécenter Difficulty (Birch speech) — forward decls
static void Task_NewGameBirchSpeech_PowerPrompt(u8 taskId);
static void Task_NewGameBirchSpeech_PowerPromptChoice(u8 taskId);
static void Task_NewGameBirchSpeech_PowerPromptChoiceHandler(u8 taskId);
static void Task_NewGameBirchSpeech_PowerYesConfirm(u8 taskId);
static void Task_NewGameBirchSpeech_PowerYesConfirmChoice(u8 taskId);
static void Task_NewGameBirchSpeech_PowerNoConfirm(u8 taskId);
static void Task_NewGameBirchSpeech_PowerNoConfirmChoice(u8 taskId);
...
static void Task_NewGameBirchSpeech_ReshowBirchLotad(u8 taskId)
{
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_PowerPrompt;
    }
}
...
// =============================
// Begin Pokécenter Difficulty Dialogue
// =============================
static void Task_NewGameBirchSpeech_PowerPrompt(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Birch_PowerRumor);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_PowerPromptChoice;
}
...
static void Task_NewGameBirchSpeech_PowerPromptChoice(u8 taskId)
{
    ...
    {
        CreateYesNoMenuParameterized(2, 1, 0xF3, 0xDF, 2, 15);
        gTasks[taskId].func = Task_NewGameBirchSpeech_PowerPromptChoiceHandler;
    }
}
...
static void Task_NewGameBirchSpeech_PowerPromptChoiceHandler(u8 taskId)
{
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
    case 0: // YES -> show "I see..." branch
        PlaySE(SE_SELECT);
        NewGameBirchSpeech_ClearWindow(0); // clear old text box first
        StringExpandPlaceholders(gStringVar4, gText_Birch_PowerYes);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_PowerYesConfirm;
        break;

    case MENU_B_PRESSED:
    case 1: // NO -> show "Oh? Then..." branch
        PlaySE(SE_SELECT);
        NewGameBirchSpeech_ClearWindow(0); // clear old text box first
        StringExpandPlaceholders(gStringVar4, gText_Birch_PowerNo);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_PowerNoConfirm;
        break;
    }
}
...
static void Task_NewGameBirchSpeech_PowerYesConfirm(u8 taskId)
{
    ...
    {
        CreateYesNoMenuParameterized(2, 1, 0xF3, 0xDF, 2, 15);
        gTasks[taskId].func = Task_NewGameBirchSpeech_PowerYesConfirmChoice;
    }
}
...
static void Task_NewGameBirchSpeech_PowerYesConfirmChoice(u8 taskId)
{
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
    case 0: // Confirm -> enable Pokécenter difficulty
        PlaySE(SE_SELECT);
        FlagSet(FLAG_POKECENTER_DIFF_MAIN);
        gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter;
        break;

    case MENU_B_PRESSED:
    case 1: // No -> go back to first prompt
        PlaySE(SE_SELECT);
        gTasks[taskId].func = Task_NewGameBirchSpeech_PowerPrompt;
        break;
    }
...
static void Task_NewGameBirchSpeech_PowerNoConfirm(u8 taskId)
{
    ...
    {
        CreateYesNoMenuParameterized(2, 1, 0xF3, 0xDF, 2, 15);
        gTasks[taskId].func = Task_NewGameBirchSpeech_PowerNoConfirmChoice;
    }
}
...
static void Task_NewGameBirchSpeech_PowerNoConfirmChoice(u8 taskId)
{
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
    case 0: // Confirm -> leave flag OFF, continue normal intro
        PlaySE(SE_SELECT);
        FlagClear(FLAG_POKECENTER_DIFF_MAIN);
        gTasks[taskId].func = Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter;
        break;
...
// =============================
// End Pokécenter Difficulty Dialogue
// =============================
